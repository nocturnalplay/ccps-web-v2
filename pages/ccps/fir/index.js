...
export default function Fir() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      let data = await IndexFetch({ table: "fir", setEvent: setStagecount });
      if (data.data.length > 0) {
        data.data.map((a, i) => {
          setStagecount((e) => ({
            ...e,
            [i == 0 ? "total" : Datalist["fir"].value[i - 1]]: a,
          }));
        });
        setStatus((a) => ({
          ...a,
        ...
        <div className="c-body">
          <div className="part-1">
            <div className="multi-list">
              If you want to access the multiple comparison in fir data list
              <Link href="/ccps/fir/table">
                <span>click here</span>
              </Link>
            </div>
            <Link href="/ccps/fir/new">
            ...
            {Object.keys(stagecount).map((a, i) => {
              if (i == 0)
                return (
                  <div className="card" key={`total-${stagecount[a]}`}>
                    ...
                    <div className="card-footer">
                      <Link
                        href={
                          date.from && date.to
                            ? `/ccps/list/fir?dt=${dateColumn}&f=${DateFormate(
                                date.from
                  ...
                  <div className="card" key={a}>
                    ...
                    <div className="card-footer">
                      <Link
                        href={
                          date.from && date.to
                            ? `/ccps/list/fir/stage?dt=${dateColumn}&v1=${a}&f=${DateFormate(
                                date.from
                              )}&t=${DateFormate(date.to)}`
                            : `/ccps/list/fir/stage?v1=${a}`
                        }
                      >
                        <h4>view list</h4>
                      </Link>
