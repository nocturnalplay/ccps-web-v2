...
import bankdata from "@/data/bank/bankComponent.json";
import banklist from "@/data/bank/bankDataFormate.json";
...
export default function Idbank({ id }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      let d = await ApiFetcher({ url: API_Query.column1("bank", "id", id) });
      if (d.Status) {
        setbank(d.Data[0]);
        setStatus((a) => ({
          ...a,
        ...
        <div className="single-view">
          <div className="single-update-info">
            <header>
              ...
              by <Link href={`/ccps/bank/update/${bank["id"]}`}>click here</Link>
            </header>
          </div>
          <PDFViewer className="pdf-view">
            <Document title={`bank-${bank["current_no"].split("/").join("-")}.pdf`}>
